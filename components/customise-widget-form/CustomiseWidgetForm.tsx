...
function CustomiseWidgetForm() {
	...
	const widgetScript = !!currentWidget
		? `<script src="https://cdn.jsdelivr.net/gh/shindeajinkya/cryptocoffeewidgetscript/widgetscript.min.js" data-name="crypto-coffee-widget" data-userid='${currentWidget.userId}' data-color='${currentWidget.widgetColor}' data-position="${currentWidget.widgetPosition}" data-x_margin="18" data-y_margin="18"></script>`
		: "";
	...
	const getCurrentUser = async () => {
		try {
			const currentUserResponse = await usersDBCollection
	...
	const handleAddressInputChange = (name: string, value: string) => {
		...
		const shortName =
			wallet_address[index].name === "Bitcoin"
				? "BTC"
				: wallet_address[index].name === "Ethereum"
				? "ETH"
				: "USDT";
		const isAddressValid =
			wallet_address[index].short_name === "SOL" ||
			WAValidator.validate(
				value,
				wallet_address[index]?.short_name ?? shortName
			) ||
			value.length == 0;
	...
	const getEditOrNewState = async () => {
		...
		try {
			...
			const widgetResponse = await widgetDBCollection
			...
			<div className="mt-10 sm:mt-0">
				<div className="md:grid md:grid-cols-3 md:gap-6">
						...
						<form onSubmit={handleSubmit(onSubmit)}>
							<div className="shadow overflow-hidden sm:rounded-md">
									...
									<div className="grid grid-cols-6 gap-6">
										...
										<div className="col-span-6">
											...
											<input
												...
												className="mt-1 focus:ring-cryptopurple focus:border-cryptopurple block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
												{...register("firstName", {
										...
										<div className="col-span-6">
											<div>
												...
												<div className="mt-1 flex rounded-md">
													...
													<input
														...
														className={
															errors.BTC
																...
																: "focus:ring-cryptopurple focus:border-cryptopurple flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300"
														}
														{...register(
															"wallet_address.0.public_address",
															{
																onChange: (e) =>
													...
													{errors.BTC ? (
														<span>
															<label className="items-center px-4 text-red-500 text-xs italic">
																{
																	errors.BTC
																		.message
																}
											...
											<div>
												<div className="mt-1 flex rounded-md">
													...
													<input
														...
														{...register(
															"wallet_address.1.public_address",
															{
																onChange: (e) =>
																	handleAddressInputChange(
																		"wallet_address.1.public_address",
											...
											<div>
												<div className="mt-1 flex rounded-md">
													...
													<input
														...
														{...register(
															"wallet_address.2.public_address",
															{
																onChange: (e) =>
																	handleAddressInputChange(
																		"wallet_address.2.public_address",
											...
											<div>
												<div className="mt-1 flex rounded-md">
													...
													<input
														...
														{...register(
															"wallet_address.3.public_address",
															{
																onChange: (e) =>
																	handleAddressInputChange(
																		"wallet_address.3.public_address",
										...
										<fieldset className="col-span-3 flex">
											<div>
											...
											<div className=" w-3/5 flex justify-around">
												<div className="flex items-center">
													<input
														...
														type="radio"
														className="focus:ring-cryptopurple h-4 w-4 text-indigo-600 border-gray-300"
														value="left"
														{...register(
															"widgetPosition",
															{ required: true }
								...
								<div className="px-4 py-3 bg-gray-50 text-right sm:px-6">
									{!!currentWidget?.isFromDb && (
										<button
											type="button"
											onClick={() =>
												setScriptModalVisible(true)
											}
											className="mr-2 inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-cryptopurple hover:bg-cryptopurple focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cryptopurple"
										>
											Show Widget Script
										</button>
									)}
									<button
										...
										className="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cryptopurple disabled:bg-gray-400"
									>
										Save
									</button>
								</div>
					...
					<div className="md:col-span-1 bg-gray-100">
						{!currentWidget ||
						!currentWidget?.wallet_address.filter(
							(wallet) => !!wallet.public_address.length
						).length ||
						!currentWidget?.firstName?.length ? (
							<div className="h-full flex flex-col items-center justify-center px-4">
								<EyeIcon width={50} height={50} />
								<div className="text-lg">Widget Preview</div>
								<div className="text-center">
									Add your Name and wallet address to see the
									preview here
								</div>
							</div>
						) : (
							<div className="flex flex-col items-center h-full">
								<WidgetComponent
									...
									availableWallets={currentWidget.wallet_address.filter(
										(wallet) =>
											!!wallet.public_address.length
									)}
								/>
							</div>
						)}
						...
						<div className="min-h-screen px-4 text-center">
							...
							>
								<div className="inline-block w-3/5 p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
									...
									>
										Copy This Script and paste into your
										site's html
									</Dialog.Title>
